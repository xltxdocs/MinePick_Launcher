...
**Текущая версия — 0.3.0**: только GUI-сборка, исполняемого файла CLI в этом репозитории нет.
...
<table>
  ...
  <tr>
    <td colspan="2"><img src="docs/screenshots/diagnosis_ru.png" width="720" alt="Диалог диагностики сбоя"/></td>
  </tr>
  <tr>
    <td colspan="2" align="center"><sub>Диагностика сбоя на размытом фоне</sub></td>
  </tr>
</table>
...
- Официальный список версий с вкладками категорий (релизы / снапшоты / первоапрельские версии / старые (legacy) версии),
  ...
  сведения о версии
- Изоляция настраивается для каждого экземпляра — всегда изолировать, никогда или по политике для
  новых экземпляров (экземпляр, в котором уже есть моды или сохранения, автоматически сохраняет свою папку)
...
- Рекомендация объёма памяти по числу модов и свободной ОЗУ, пользовательские аргументы JVM, прямое подключение
  к серверу, язык игры, поведение «после запуска игры», освобождение памяти лаунчера
- Экземпляры — это папки версий: каждая установленная версия *и есть* экземпляр, поэтому ничего не
  дублируется и не нужно создавать заранее. У каждого свои сохранения / моды / конфигурация и свои
  переопределения изоляции, Java, памяти, аргументов JVM и дополнительных аргументов игры — любое из
  них может наследовать глобальное значение или сбрасываться одним действием
- Страница экземпляров — это список и подробный просмотр с вкладками «Обзор», «Моды», «Ресурсы»,
  «Сохранения», «Настройки» и «Диагностика»; у экземпляров есть заметки, их можно переименовывать,
  экспортировать и импортировать, а локальный менеджер модов читает метаданные jar
  (Fabric / Quilt / NeoForge / Forge / mcmod.info) и поддерживает включение и отключение, поиск с
  фильтром и перетаскивание

### Диагностика сбоев и запуска
- Если запуск не удался или игра завершилась аварийно, лаунчер читает лог игры, отчёты о сбоях,
  файл `hs_err` и вывод запуска, за которым он только что следил, сопоставляет их со встроенной
  базой знаний и объясняет, **что, вероятно, произошло, почему и что с этим делать**, с устойчивым
  кодом ошибки (`MPL-Launch-####` для сбоев на стороне лаунчера, `MPL-Crash-####` для сбоев на
  стороне игры)
- Объяснение показывается в диалоговом окне поверх размытого фона; его можно экспортировать в виде
  отчёта (токены доступа и пути пользователя маскируются) или скопировать как код с кратким описанием;
  автоматический анализ и размытие можно отключить по отдельности, а каждая диагностика сохраняется
  для своего экземпляра. Анализ выполняется локально — ничего не отправляется в сеть
...
- Страница **«О программе»**: установленная версия, ссылка на исходный код, лицензия и правовая информация,
  благодарности третьим сторонам и проверка обновлений (сравнение с последним релизом на GitHub)
- **Четыре режима автообновления**: скачать и установить, скачать и уведомить (по умолчанию), только уведомить,
  не проверять автоматически; если установка невозможна, открывается страница релиза

## Загрузка и использование

Скачайте `MinePick_Launcher.exe` со страницы Releases и запустите двойным щелчком — установщик не нужен,
...
- [MinePick Launcher Classic](https://github.com/xltxdocs/MinePick_Launcher_Classic) — предшественник этого проекта (архивирован, GUI + CLI)
