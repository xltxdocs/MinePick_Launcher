...
# MinePick Launcher — пробная версия интерфейса
...
**Этот репозиторий — экспериментальная линия интерфейса (версия 0.1.2).** В нём тот же набор функций
...
- **Мастер первого запуска прямо в окне** — приветственный сценарий идёт оверлеем внутри главного окна
  (слева шкала шагов, справа содержимое, снизу панель действий), а не отдельным диалоговым окном;
  по завершении он плавно скрывается
- **Собственная рамка окна** — заголовок окна рисует сам лаунчер, поэтому оформление совпадает с темой;
  значок окна во всех случаях и при любом размере — дизайн кирки
- **Настраиваемый внешний вид** — акцентный цвет (любое значение HEX, восемь пресетов или системный
  выбор цвета), шрифт интерфейса (все установленные семейства, частые закреплены, фильтрация при вводе),
  скругление углов (компактное / обычное / скруглённое), тема (тёмная / светлая / как в системе)
- **Более понятная вёрстка** — на каждой странице заголовок с описанием в одну строку, блок с брендом
  над боковой панелью, единая шкала отступов на всех страницах, значки лупы в полях поиска
- **Спокойнее взаимодействие** — полосы прокрутки появляются, только когда указатель находится внутри
...
- **Понятные состояния** — кнопки разделены по важности (основная / второстепенная / контурная для опасных действий),
  сообщения о состоянии окрашены по серьёзности, пустые списки подсказывают, что делать дальше
...
### Аккаунты
- Вход Microsoft по потоку кода устройства (страница авторизации открывается автоматически, а код
  копируется в буфер обмена), со статусом опроса в реальном времени
...
### Версии и Java
- Официальный список версий с вкладками категорий (релизы / снапшоты / первоапрельские / устаревшие),
...
### Ресурсы
- Моды, ресурс-паки, шейдеры и модпаки из **Modrinth** и **CurseForge**, топ-30 популярных ресурсов на каждой
  вкладке, поиск по ключевым словам (включая названия из китайского сообщества), установка в один клик,
...
Сборка и подпись: `pyinstaller build_exe.spec` → `scripts/sign_exe.ps1` (см. `docs/code_signing.md`).
Спецификация создаёт один GUI EXE (предварительные требования для сборки перечислены в `docs/github_release.md`).

Проверка интерфейса одной командой (тесты + lint + контраст/переполнение/высокий DPI + аудит углов + скриншоты):

```powershell
python tools/ui_regression.py            # add --quick to skip the screenshots
```

Внутреннее устройство тем (плейсхолдеры, хуки настройки, как добавить новый параметр): `docs/theming.md`.
