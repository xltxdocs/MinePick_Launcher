...
def test_required_java_major_mapping():
    ...
    assert required_java_major("26.3-snapshot-8") == 25  # snapshot baseline version
    assert required_java_major("1.20.5-pre1") == 21  # pre-releases use the baseline version
    assert required_java_major("1.22") is None  # out of range (theoretically nonexistent) -> fall back to the declared value
    assert required_java_major("26.0") is None  # nonexistent version -> None
    assert required_java_major("25w01a") is None  # snapshots cannot be parsed by semver
    # declared value takes precedence
