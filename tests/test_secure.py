"""Token encryption vault and AccountStore encryption/decryption tests."""
...
def test_vault_env_password(ws_tmp, monkeypatch):
    ...
    secure.forget_password()  # relies only on the environment variable
...
def test_account_store_encryption_roundtrip(ws_tmp, monkeypatch):
    ...
    # after disabling encryption, writes back plaintext
...
def test_cli_vault_error_no_traceback(ws_tmp, monkeypatch, capsys):
    """When token encryption is on but no password is provided, the CLI prints a friendly error and returns 1 (no traceback)."""
    from launcher import config as config_mod
    from launcher.auth.models import Account, MicrosoftTokens
    ...
    store.save({"id1": account})
    secure.forget_password()  # simulate the locked state
...
def test_cli_offline_login_locked(ws_tmp, monkeypatch, capsys):
    """Offline login is blocked by the gate: exit code 1 + a friendly message (no account created)."""
    from launcher import config as config_mod
    from launcher.cli import main
