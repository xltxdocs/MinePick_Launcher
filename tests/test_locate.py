...
def test_match_java_fallback():
    runtimes = [_rt(8)]
    assert match_java(runtimes, 17).major == 8  # none satisfies, falls back to the highest
...
def test_has_suitable_java():
    ...
    assert has_suitable_java([_rt(21), _rt(25)], 8) is False  # old versions require exactly Java 8
    assert has_suitable_java([_rt(8)], 8) is True
    assert has_suitable_java([_rt(21)], 17) is True  # 17+ uses >= matching
