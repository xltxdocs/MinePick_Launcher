...
def test_fabric_versions():
    ...
    assert [v.version for v in versions] == ["0.19.3", "0.19.2"]  # 仅 stable，新在前
...
def test_neoforge_versions():
    ...
    assert [v.version for v in v2612] == ["26.1.2.95", "26.1.2.94"]  # 新在前
...
def test_neoforge_game_versions():
    ...
    assert "26.1.2.95" not in games  # 构建号不出现
