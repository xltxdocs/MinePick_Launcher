...
def _offline_manifest_fetch(monkeypatch):
    """版本页启动时自动拉清单：测试中改为静默失败，避免真实网络请求。"""
    def _fail(**kw):
...
def test_versions_page_categories_and_full_list(app, monkeypatch, ws_tmp):
    ...
    try:
        ...
        page._on_manifest(SimpleNamespace(versions=versions, latest={}))
        # 全部列出，无 500 行上限
        ...
        # 过滤下拉框：全部/正式/快照/愚人节/旧版
        combo = page.type_combo
        assert combo.count() == 5
        assert [combo.itemText(i) for i in range(combo.count())] == [
            "全部类型", "正式版", "快照版", "愚人节版本", "远古版"
        ]
        combo.setCurrentIndex([combo.itemData(i) for i in range(combo.count())].index("april_fools"))
        assert page.table.rowCount() == 1
        assert page.table.item(0, 0).text() == "24w14potato"
        combo.setCurrentIndex([combo.itemData(i) for i in range(combo.count())].index("release"))
        assert page.table.rowCount() == 550
        combo.setCurrentIndex([combo.itemData(i) for i in range(combo.count())].index("snapshot"))
        assert page.table.rowCount() == 1
        assert page.table.item(0, 0).text() == "25w14a"
        combo.setCurrentIndex([combo.itemData(i) for i in range(combo.count())].index("legacy"))
        assert page.table.rowCount() == 1
