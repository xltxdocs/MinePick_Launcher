...
def test_login_interactive_reports_flow_callback():
    """After getting the device code, call back on_flow (the GUI uses it to show a countdown)."""
    session = ms.MicrosoftSession(
...
def test_ensure_session_refreshes_mc_with_valid_ms():
    _mock_chain_ok()
    account = _account(mc_exp=time.time() - 10)  # mc token expired, ms still valid
    session = ms.MicrosoftSession(flow=FakeDeviceFlow())
    ...
    assert updated.tokens.ms_access_token == "ms-at"  # no ms refresh triggered
