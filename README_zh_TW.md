...
<table>
  ...
  <tr>
    <td colspan="2"><img src="docs/screenshots/diagnosis_zh_TW.png" width="720" alt="崩潰診斷對話方塊"/></td>
  </tr>
  <tr>
    <td colspan="2" align="center"><sub>崩潰診斷，背景為模糊效果</sub></td>
  </tr>
</table>
...
- 隔離由每個實例各自決定 —— 始終隔離、不隔離，或跟隨新實例的預設策略
  （已含有模組或存檔的實例會自動保留自己的資料夾）
...
- 依模組數量與可用記憶體提供記憶體建議、自訂 JVM 參數、伺服器直連、遊戲語言、
  「啟動遊戲後」行為、釋放啟動器記憶體
- 實例就是版本資料夾：每個已安裝的版本「本身」就是一個實例，因此不會有重複的檔案，
  也不必事先建立。每個實例各自保留自己的存檔 / 模組 / 設定，以及專屬的隔離、Java、記憶體、
  JVM 參數與額外遊戲參數覆寫設定 —— 每一項都能跟隨全域值，或一鍵重設
- 實例頁面是清單加上詳細檢視，並以頁籤區分概覽、模組、資源、存檔、設定與診斷；實例支援備註、
  重新命名與匯入/匯出，本機模組管理會讀取 jar 中介資料（Fabric / Quilt / NeoForge / Forge /
  mcmod.info），支援啟用/停用、搜尋與篩選及拖放

### 崩潰與啟動診斷
- 當啟動失敗或遊戲異常結束時，啟動器會讀取遊戲日誌、崩潰報告、任何 `hs_err` 檔案，
  以及它剛剛監看的那次執行的輸出，並與內建知識庫比對，說明**可能發生了什麼事、為什麼，
  以及該怎麼處理**，同時附上穩定的錯誤碼（啟動器端的失敗為 `MPL-Launch-####`，
  遊戲端的崩潰為 `MPL-Crash-####`）
- 說明會顯示在模糊背景上的對話方塊中，並可匯出為診斷報告（存取權杖與使用者路徑會加以遮蔽）
  或複製錯誤碼與摘要；自動分析與背景模糊都能各自關閉，而且每次診斷都會依實例記錄。
  分析完全在本機進行 —— 不會上傳任何內容
