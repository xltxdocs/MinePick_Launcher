...
### Launching
- Memory (manual or auto-allocate from mod count and available RAM), game language, custom JVM arguments, direct server connect, demo mode
- Live game log tail and a crash-report viewer
- After-launch launcher window: keep open / auto-hide / auto-exit
- Release launcher memory after the game starts (on by default); low-RAM warning before launch
...
- Mod / resource pack / shader search & install (keyword search, download counts, version/loader selection)
- One-click modpack install (auto loader setup → instance creation → all files + overrides merged)
- Chinese keyword search via a built-in community-name table
...
- "View Crash Reports" button opens the selected instance's crash folder
...
- Dark / light themes; visually hidden scrollbars (wheel scrolling everywhere)
- First-run wizard (language / game directory / memory)
- Download speed limit, resumable downloads, live speed & ETA
- Performance: virtualized version list, debounced search, cached popular/config data, lazy loading and a shared HTTP connection pool
...
- [MinePick Launcher Revision](https://github.com/TheDarkLord234/MinePick_Launcher_Revision) — a revised edition of MinePick Launcher by a fellow community member
