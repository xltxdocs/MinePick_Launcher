...
**이 저장소는 인터페이스 시험 라인(버전 0.1.2)입니다.** 메인 프로젝트와 동일한 런처 기능 세트에
...
- **창 안에서 진행되는 최초 실행 마법사** — 환영 흐름이 별도 대화 상자가 아니라 메인 창 내부의 오버레이
  (왼쪽에 단계 레일, 오른쪽에 콘텐츠, 아래쪽에 작업 표시줄)로 표시되며, 완료하면
  자연스럽게 사라집니다
- **사용자 지정 창 프레임** — 제목 표시줄을 런처가 직접 그리므로 창 테두리가 테마와
  일치합니다. 창 아이콘은 모든 위치, 모든 크기에서 곡괭이 디자인입니다
- **사용자 지정 가능한 모양** — 강조 색상(임의의 16진수 값, 8가지 프리셋 또는 시스템 색 선택기),
  인터페이스 글꼴(설치된 모든 글꼴 패밀리, 자주 쓰는 글꼴 고정, 입력하여 필터링), 모서리 둥글기(좁게 / 기본 /
  둥글게), 테마(다크 / 라이트 / 시스템 설정 따르기)
- **더 명확해진 레이아웃** — 모든 페이지에 한 줄 설명이 있는 페이지 헤더, 사이드바 위의
  브랜드 블록, 모든 페이지에 통일된 간격 체계, 검색 필드의 돋보기 아이콘
- **차분해진 상호작용** — 스크롤 막대는 포인터가 목록 안에 있을 때만 나타나고, 포커스 외곽선은
  키보드 탐색에만 표시되며, 페이지를 전환할 때 짧은 페이드가 적용되고, 표 열 너비가 기억되며,
  헤더를 클릭해 정렬할 수 있습니다
- **읽기 쉬운 상태** — 버튼에 위계가 있고(기본 / 보조 / 외곽선 위험), 상태 메시지는
  심각도에 따라 색이 달라지며, 빈 목록은 다음에 무엇을 해야 할지 알려줍니다
- **기본으로 제공되는 접근성** — 모든 텍스트/배경 조합이 WCAG AA 명도 대비를 충족하며(강조 색상 위의
  흰색 채우기는 자동으로 어두워짐), 인터페이스는 9개 언어로 제공됩니다
...
- 버전 격리: 각 버전이 자체 저장 / 모드 / 설정을 유지합니다
- Java 요구 사항 매핑(1.16.5→8, 1.17–1.20.4→17, 1.20.5–1.21.11→21, 26.1+→25)과 Adoptium
  자동 다운로드 및 런타임 관리자

### 실행 & 인스턴스
- 모드 수와 사용 가능한 RAM을 기준으로 한 메모리 추천, 사용자 지정 JVM 인수, 서버 직접 연결, 게임 언어,
  실시간 로그 추적, “게임 실행 후” 동작, 런처 메모리 해제(작업 세트 트리밍)
...
### 리소스
- **Modrinth**와 **CurseForge**의 모드, 리소스팩, 셰이더, 모드팩, 탭별 인기 Top 30,
  키워드 검색(중국 커뮤니티 이름 포함), 원클릭 설치, `.mrpack` 모드팩 설치
...
빌드 및 서명: `pyinstaller build_exe.spec` → `scripts/sign_exe.ps1` (`docs/code_signing.md` 참고).
이 spec 파일은 단일 GUI EXE를 생성합니다(빌드 사전 요구 사항은 `docs/github_release.md`에 나열되어 있습니다).
...
테마 내부 구조(플레이스홀더, 사용자 지정 훅, 새 옵션을 추가하는 방법): `docs/theming.md`.
