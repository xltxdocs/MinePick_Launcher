...
**현재 버전 0.3.1** —— GUI 빌드만 제공하며, 이 저장소에는 CLI 실행 파일이 없습니다.
...
<table>
  ...
  <tr>
    <td colspan="2"><img src="docs/screenshots/diagnosis_ko.png" width="720" alt="크래시 진단 대화 상자"/></td>
  </tr>
  <tr>
    <td colspan="2" align="center"><sub>크래시 진단, 흐리게 처리된 배경 위에서</sub></td>
  </tr>
</table>
...
- 격리 여부는 인스턴스마다 결정됩니다 — 항상 격리, 격리 안 함, 또는 새 인스턴스에 정책 적용
  (이미 모드나 세이브가 있는 인스턴스는 자동으로 자체 폴더를 유지합니다)
...
- 인스턴스는 곧 버전 폴더입니다: 설치된 모든 버전이 *인스턴스*이므로 중복이 생기지 않고, 미리
  만들어 둘 필요도 없습니다. 각 인스턴스는 자체 세이브 / 모드 / 설정을 유지하며, 격리, Java, 메모리,
  JVM 인수, 추가 게임 인수를 인스턴스별로 재정의할 수 있습니다 — 각 항목은 전역 값을 따르게 하거나
  한 번의 동작으로 초기화할 수 있습니다
- 인스턴스 페이지는 목록과 상세 보기로 구성되며, 상세 보기에는 개요, 모드, 리소스, 세이브, 설정,
  진단 탭이 있습니다. 인스턴스에는 메모를 남길 수 있고 이름 변경, 내보내기/가져오기를 지원하며,
  로컬 모드 관리자는 jar 메타데이터(Fabric / Quilt / NeoForge / Forge / mcmod.info)를 읽고
  활성화/비활성화, 검색 및 필터, 드래그 앤 드롭을 지원합니다

### 크래시 및 실행 진단
- 실행에 실패하거나 게임이 비정상 종료되면, 런처는 게임 로그와 크래시 보고서, `hs_err` 파일,
  방금 지켜본 실행의 출력을 읽어 내장 지식 베이스와 대조한 뒤, **무엇이 일어났을 가능성이 높은지,
  왜 그런지, 어떻게 해야 하는지**를 안정적인 오류 코드와 함께 설명합니다
  (런처 측 실패는 `MPL-Launch-####`, 게임 측 크래시는 `MPL-Crash-####`)
- 설명은 흐리게 처리된 배경 위 대화 상자에 표시되며, 보고서로 내보내거나(액세스 토큰과 사용자
  경로는 마스킹됩니다) 코드와 요약으로 복사할 수 있습니다. 자동 분석과 배경 흐리기는 각각 끌 수
  있고, 모든 진단은 인스턴스별로 기록됩니다. 분석은 로컬에서 이루어지며 아무것도 업로드되지 않습니다
