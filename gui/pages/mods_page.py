...
from launcher.mods.loaders import display_loader_name
    ...
    def _fill(self, versions: list[ModVersion]) -> None:
        ...
        for row, version in enumerate(versions):
            ...
            self.table.setItem(
                row,
                1,
                QTableWidgetItem(", ".join(display_loader_name(l) for l in version.loaders)),
            )
...
class _ContentTab(QWidget):
    ...
    def __init__(self, kind: str) -> None:
        ...
        for name in ("fabric", "forge", "neoforge", "quilt"):
            self.loader_combo.addItem(display_loader_name(name), name)
        ...
        self.popular_button.setObjectName("secondaryButton")
        ...
        self.empty_results = EmptyState(tr("empty.mods"))
        left.addWidget(self.empty_results)
    ...
    def _set_entries(self, entries: list[object]) -> None:
        ...
        self.empty_results.update_for(len(entries))
        if entries:
...
class ResourcesPage(QWidget):
    def __init__(self) -> None:
        ...
        layout.addWidget(build_page_header(tr("nav.mods"), tr("page.mods.desc")))
