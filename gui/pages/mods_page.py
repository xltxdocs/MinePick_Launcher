"""资源页：模组搜索（Modrinth）/ 整合包 / 资源包 / 光影 + 已安装内容管理。"""
...
class _VersionPickerDialog(QDialog):
    """Modrinth 项目版本选择弹窗（版本号 / 加载器 / MC 版本）。"""
...
class _ContentTab(QWidget):
    """通用资源选项卡。kind: mod / modpack / resourcepack / shaderpack"""
    ...
    def __init__(self, kind: str) -> None:
        super().__init__()
        self.kind = kind
        self._entries: list[object] = []  # ModSearchHit（搜索）或 ModInfo（slug 查询）
        ...
        search_row.addWidget(self.popular_button)
        ...
        if kind != "modpack":
            ...
            apply_no_focus_outline(self.installed_list)
            self.installed_list.setFixedHeight(110)
            self.installed_refresh_button = QPushButton(tr("mods.installed.refresh"))
            self.installed_delete_button = QPushButton(tr("mods.installed.delete"))
            installed_row = QHBoxLayout()
            installed_row.addWidget(QLabel(tr("mods.installed.label")))
            installed_row.addWidget(self.installed_refresh_button)
            installed_row.addWidget(self.installed_delete_button)
            installed_row.addStretch(1)
            layout.addLayout(installed_row)
            layout.addWidget(self.installed_list)
            self.installed_refresh_button.clicked.connect(self.refresh_installed)
            self.installed_delete_button.clicked.connect(self._delete_installed)
    ...
    def _search(self) -> None:
        ...
        def do_search() -> object:
            ...
            if has_cjk(query):
                # 中文查询：本地中文名表匹配置顶，再合并 Modrinth 结果
                local_hits = []
    ...
    def load_popular(self) -> None:
        """加载该类型的下载量 Top 30；首次切到选项卡时自动加载一次。"""
    ...
    def _install_cf(self, hit: ModSearchHit) -> None:
        """从 CurseForge 安装：取适配游戏版本的最新正式版文件，下载到内容目录。"""
    ...
    def refresh_installed(self) -> None:
        ...
        items = list_installed_content(game_dir, subdir, **args)
        self.installed_list.clear()
        if not items:
            self.installed_list.addItem(QListWidgetItem(tr("mods.installed.empty")))
            return
        for item in items:
            self.installed_list.addItem(
                QListWidgetItem(item.name + "   (" + _format_size(item.size) + ")")
            )
    ...
    def _delete_installed(self) -> None:
        ...
        if item is None or item.text() == tr("mods.installed.empty"):
            return
        name = item.text().split("   (")[0]
        answer = QMessageBox.question(
            self,
            tr("mods.installed.delete.dialog"),
            tr("mods.installed.delete.msg", name),
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            QMessageBox.StandardButton.No,
        )
        if answer != QMessageBox.StandardButton.Yes:
        ...
        game_dir = args.pop("game_dir")
        try:
            delete_installed_content(game_dir, subdir, name, **args)
        except ModsError as exc:
            self.status.setText(tr("mods.installed.msg.delete_fail", str(exc)))
            return
        self.refresh_installed()
        self.status.setText(tr("mods.installed.msg.deleted", name))
...
class ResourcesPage(QWidget):
    def __init__(self) -> None:
        ...
        self.source_combo.addItem(tr("mods.source.modrinth"), "modrinth")
        self.source_combo.addItem(tr("mods.source.curseforge"), "curseforge")
        self.tabs = QTabWidget()
        # 去掉样式引擎自带的选项卡底部基线，保持完全扁平
        self.tabs.tabBar().setDrawBase(False)
        self.tabs.tabBar().setFocusPolicy(Qt.NoFocus)  # 去掉选中选项卡文字的焦点框
        ...
        for tab in (
            ...
            tab.page = self
        self.mods_tab.load_popular()  # 默认第一个选项卡先加载热门

        title_row = QHBoxLayout()
        title_row.addWidget(self.title)
        title_row.addStretch(1)
        title_row.addWidget(QLabel(tr("mods.source")))
