...
class _ContentTab(QWidget):
    ...
    def _search(self) -> None:
        ...
        project_type = {"resourcepack": "resourcepack", "shaderpack": "shader"}.get(
            self.kind, "mod"
        )

        def do_search() -> object:
            return search_projects(query, limit=30, project_type=project_type)
    ...
    def _on_install_clicked(self) -> None:
        if self.kind == "mod" and self._hit is not None:
            self._pick_version()
            return
        self._on_install()
