...
class VersionListModel(QAbstractTableModel):
    ...
    def sort(self, column: int, order=Qt.SortOrder.AscendingOrder) -> None:
        """Header-click sorting: rows are plain tuples, so a key sort is enough."""
        if not self._rows or not 0 <= column < COLUMNS:
            return
        self.layoutAboutToBeChanged.emit()
        self._rows.sort(key=lambda row: row[column], reverse=order == Qt.SortOrder.DescendingOrder)
