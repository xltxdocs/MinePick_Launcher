...
- **交互更安静** —— 滚动条只在鼠标移入列表时出现、焦点描边只对键盘导航显示、切页有淡入过渡、
  表格列宽会被记住、点表头即可排序
- **状态更易读** —— 按钮分三级（主操作 / 次要描边 / 危险红描边）、状态文字按严重程度着色、空列表会告诉你怎么开始
- **默认可访问** —— 所有文字与背景的对比度达 WCAG AA（承载白字的填充色自动加深），界面支持 9 种语言
...
- Java 需求映射（1.16.5→8、1.17–1.20.4→17、1.20.5–1.21.11→21、26.1+→25），支持 Adoptium 自动下载与运行时管理
...
- 实例隔离，支持备注、重命名、导入导出；实例内嵌本地模组管理（读取 Fabric / Quilt / NeoForge / Forge /
  mcmod.info 元数据，一键启用禁用、搜索筛选、拖入安装）
...
> 本构建使用自签名证书（WDNDXLTX），他人电脑的 SmartScreen 可能提示「未知发布者」，
> 点「更多信息 → 仍要运行」即可。
