"""Microsoft account login: OAuth device-code flow + Xbox auth chain + Minecraft session.

Auth chain (see wiki.vg / Microsoft Authentication Scheme):
  MS device-code flow -> MS access token
  -> XBL (user.auth.xboxlive.com) -> XSTS (xsts.auth.xboxlive.com)
  -> Minecraft access token (api.minecraftservices.com)
  -> entitlement check -> game profile (UUID/username)
"""
...
# Microsoft has retired the old public client id (e.g. 00000000402b5328, which returns AADSTS700016).
# The current default comes from the build config in the PrismLauncher open-source project (GPL) source, and supports the device-code flow;
# override via config msa_client_id or the MSA_CLIENT_ID environment variable (e.g. after registering your own Azure app).
...
# Note: msal appends reserved scopes like offline_access automatically; passing them explicitly is rejected
...
class AuthError(Exception):
    """Base class for auth failures (user-facing message)."""
...
def open_verification_page(flow: dict[str, Any]) -> bool:
    """Open the device-code verification page in the default browser (best-effort; failure doesn't affect login)."""
    uri = flow.get("verification_uri")
    if not uri:
        return False
    try:
        return bool(webbrowser.open(str(uri)))
    except Exception:  # noqa: BLE001 - a failed open doesn't affect the login flow
        return False


def copy_user_code(flow: dict[str, Any]) -> bool:
    """Copy the device code to the system clipboard (best-effort; failure doesn't affect login).

    Windows uses PowerShell Set-Clipboard (no pipe, no console flash);
    macOS uses pbcopy; Linux tries xclip/xsel (both need a pipe; skipped automatically in restricted environments).
    """
    code = str(flow.get("user_code") or "")
    if not code or re.fullmatch(r"[A-Za-z0-9-]+", code) is None:
        return False  # don't inject unusual characters into the command line
    try:
        if os.name == "nt":
            extra: dict = {}
...
class XboxAuthChain:
    """HTTP auth chain for XBL -> XSTS -> Minecraft session."""
    ...
    def minecraft_token(self, uhs: str, xsts_token: str) -> tuple[str, int]:
        """Return (minecraft access token, validity in seconds)."""
        resp = self._client.post(
            MC_LOGIN_URL,
...
class DeviceFlowProtocol(Protocol):
    """Interface for the msal device-code flow (for test injection)."""
...
class MicrosoftSession:
    """Combine the device-code flow and the Xbox auth chain to produce/refresh a Minecraft session."""
    ...
    def login_interactive(
        ...
        """Interactive login: show the device code and block waiting for user authorization.

        The progress callback is for UI/logging; on_flow is called once after the device
        code is obtained (carrying fields like expires_in / interval for the GUI countdown).
        """
    ...
    def ensure_session(self, account: Account) -> Account:
        """Ensure the Minecraft session is usable: refresh tokens and re-run the auth chain when needed, returning the possibly-updated account."""
