"""Adoptium Temurin JRE 下载与安装（api.adoptium.net v3）。

托管运行时目录布局：<launcher 数据目录>/runtime/java-<major>/bin/java(.exe)。
下载链接最终指向 github.com（Adoptium 发布资产），需系统证书库（见 meta.manifest._enable_system_ca）。
"""
...
class JavaAsset:
    ...
    name: str
    size: int
    checksum: str = ""  # 官方 sha256（package.checksum，#17 完整性校验）
...
def _extract_zip(zip_path: Path, target_dir: Path) -> None:
    """解压 zip 到目标目录，去除 zip 内顶层目录（Adoptium 包结构）。"""
...
def list_managed_runtimes(
    ...
    """列出托管 Java 运行时（major, 目录），按 major 升序。"""
...
def install_java(
    ...
    *,
    ...
    """下载并安装 Adoptium JRE 到托管目录；已安装则直接返回。"""
    from launcher import paths as launcher_paths
