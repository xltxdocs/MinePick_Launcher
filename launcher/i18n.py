...
CORE_TRANSLATIONS: dict[str, dict[str, str]] = {
    ZH: {
        ...
        "instances.import_invalid": "不是有效的实例压缩包（缺少 minepick.json）",
        "instances.field_unknown": "未知的实例字段: {}",
        "instances.source_missing": "源版本未安装: {}",
        ...
        "offline.locked": "离线模式需先使用微软正版账号登录一次（仅当系统语言为简体中文、地区为中国大陆且启动器语言为简体中文时可直接使用）",
    ...
    EN: {
        ...
        "instances.import_invalid": "Not a valid instance archive (missing minepick.json)",
        "instances.field_unknown": "Unknown instance field: {}",
        "instances.source_missing": "Source version is not installed: {}",
        ...
        "offline.locked": "Offline mode requires signing in with a Microsoft account first (available without signing in only when the system language is Simplified Chinese, the region is mainland China and the launcher language is Simplified Chinese)",
...
def describe_network_error(exc: httpx.HTTPError) -> str:
    """Map an httpx exception to a user-facing message (network errors / 5xx)."""
