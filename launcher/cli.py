...
子命令：paths / config / login / logout / whoami / list-versions / show /
...
GUI 对应：python -m gui 或 MinePick_Launcher.exe（无参数启动）。
"""
...
def cmd_login(args: argparse.Namespace) -> int:
    ...
    store.save(accounts)
    _select_account(account.id)
    config.unlock_offline_mode()  # 正版登录后解锁离线模式
