"""加载器版本源：Fabric / Forge / NeoForge（统一 LoaderVersion 抽象）。"""
...
def _version_key(value: str) -> tuple:
    """数字段优先、非数字段兜底的排序键（如 20.2.12-beta）。"""
...
def _neoforge_base(game_version: str) -> str:
    """MC 版本 -> NeoForge 版本前缀（1.20.2 -> 20.2；26.1.2 -> 26.1.2）。"""
...
def _neoforge_versions(
    ...
    versions = [
        ...
        and "-" not in node.text  # 跳过 beta/rc 等预发布构建
    ]
    versions.sort(key=_version_key)
    out: list[LoaderVersion] = []
    for version in reversed(versions[-20:]):  # 新在前
        out.append(
            LoaderVersion(
                loader="neoforge",
                version=version,
...
def list_loader_versions(
    ...
    """列出指定加载器对指定 MC 版本的可用版本（新到旧）。"""
    if loader == "fabric":
        return _fabric_versions(game_version, client)  # API 已按新到旧返回
    if loader == "forge":
        return list(reversed(_forge_versions(game_version, client)))  # latest 在前
    if loader == "neoforge":
        return _neoforge_versions(game_version, client)  # 已按新到旧
