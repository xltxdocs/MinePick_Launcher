"""整合包（Modrinth .mrpack）下载与安装。

流程：下载 .mrpack -> 解析 modrinth.index.json -> 自动安装所需加载器
-> 创建同名实例 -> 下载全部文件 + 合并 overrides。
"""
...
def _ensure_loader_profile(
    ...
    """确保加载器档案已安装；缺失则自动用官方安装器安装。返回档案 id。"""
...
def install_modpack(
    ...
    *,
    ...
    if mod_version_id:
        ...
        # 优先选择本机已安装 MC 版本对应的构建（下载后核对索引里的 MC）
        ...
        for candidate in ordered[:5]:
            ...
            if mc in installed:
                ...
                break
            index, overrides_data = idx, ov  # 保留最后一个作为兜底
        if index is None:
    ...
    except Exception:  # noqa: BLE001 - 名称非法时退回默认名
